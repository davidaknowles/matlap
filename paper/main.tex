...
$\ell=\log \sigma$:
\begin{equation} 
    \frac{\partial}{\partial \ell} \log |A_i| = \operatorname{tr} \left(A_i^{-1}\frac{\partial A_i}{\partial \ell} \right). \approx \frac{1}{R} \sum_{r=1}^R z_{ir}^\top A_i^{-1} \frac{\partial A_i}{\partial \ell} z_{ir}.
\end{equation}
where $z_{ir}$ are $R$ (fixed) Hutchinson probes satisfying $\mathbb{E}(z_{ir}z_{ir}^\top)=I$. Each term requires solving
\begin{equation} 
    A_i x_{ir} = \frac{\partial A_i}{\partial \ell}z_{ir},
\end{equation}
which can be done by conjugate gradients using only matrix-vector products
\begin{equation} 
    x \mapsto \operatorname{diag}\{p_{ij}(\sigma)\}x + \eta B(\hat X)x.
\end{equation}
